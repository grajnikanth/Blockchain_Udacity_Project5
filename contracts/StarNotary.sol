...
contract StarNotary is ERC721 {
    ...
    // unit256 will be the token ID and at that Token ID the corresponding struct of Star is stored
    mapping(uint256 => Star) public tokenIdToStarInfo;
    // mapping the TokenId and price
    // At the given Token ID of Star store the corresponding price of that star for sale. 
    // The price is in ETHER??? 
    // TokenID is uint256 and price is also uint256 
    mapping(uint256 => uint256) public starsForSale;

    
    // Create Star using the Struct
    function createStar(string memory _name, uint256 _tokenId) public { // Passing the name and tokenId as a parameters
        Star memory newStar = Star(_name); // Star is an struct so we are creating a new Star in memory
        tokenIdToStarInfo[_tokenId] = newStar; // new Star -> tokenId mapping
    ...
    function exchangeStars(uint256 _tokenId1, uint256 _tokenId2) public {
        ...
        require(ownerOf(_tokenId1) == msg.sender || ownerOf(_tokenId2) == msg.sender,"You are not the owner of either stars. You cannot exchange");
